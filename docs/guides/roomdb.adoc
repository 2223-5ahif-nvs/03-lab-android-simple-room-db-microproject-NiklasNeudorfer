...
<.> This is the List where the data gets saved -> in the composable you need to access this list.

<.> This is how you can access the database from a Composable and convert the `Flow` to a `List<Author>`
...
[NOTE]
The `nukeTable` function I have in the Screenshot above is only uncommented when I want to use it!
Directly after that it should be commented out again to prevent accidental deletion of all data!
Another way would be to just use the DB-Inspector console
